...
if version_info >= (2, 6, 0):
    def swig_import_helper():
        ...
        fp = None
        try:
            fp, pathname, description = imp.find_module('_solar_continuum_table', [dirname(__file__)])
        except ImportError:
            import _solar_continuum_table
            return _solar_continuum_table
        if fp is not None:
            try:
                _mod = imp.load_module('_solar_continuum_table', fp, pathname, description)
            finally:
                fp.close()
            return _mod
    _solar_continuum_table = swig_import_helper()
...
_solar_continuum_table.SHARED_PTR_DISOWN_swigconstant(_solar_continuum_table)
SHARED_PTR_DISOWN = _solar_continuum_table.SHARED_PTR_DISOWN
...
class SolarContinuumTable(full_physics_swig.solar_continuum_spectrum.SolarContinuumSpectrum):
    ...
    __setattr__ = lambda self, name, value: _swig_setattr(self, SolarContinuumTable, name, value)
    __swig_getmethods__ = {}
    for _s in [full_physics_swig.solar_continuum_spectrum.SolarContinuumSpectrum]:
        __swig_getmethods__.update(getattr(_s, '__swig_getmethods__', {}))
    __getattr__ = lambda self, name: _swig_getattr(self, SolarContinuumTable, name)
    __repr__ = _swig_repr
...
SolarContinuumTable_swigregister(SolarContinuumTable)
